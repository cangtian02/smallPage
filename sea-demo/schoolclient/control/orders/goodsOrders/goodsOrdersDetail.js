define(function(require, exports, module) {
	...
	window.jsObj.setLoadUrlTitle('商品订单详情');
	...
	ajax.get('order/orderpaydetails/' + orderIds + '/' + customerId,'json',function(data){
		...
		for(var i = 0; i < orderList.length; i++){
			...
			for(var k = 0; k < goodsList.length; k ++){
				...
				lidata += '<a href="javascript:;" data-url="' + SAYIMO.SRVPATH + htmlUrl + '?goodsId=' + goodsList[k].id + '&normsValueId=' + goodsList[k].normsValueId + ahref + '" class="cont">' + 
				'	<div class="l">' + 
				...
				'		</div>' + 
				'		<div class="num ellipsis">' + goodsList[k].normsValues + ' x' + goodsList[k].buyNum + '</div>' + 
				'	</div>' + 
				'</a>';					
			}
			var leaveWords = '';
	...
	function control(){
		...
		$("#payment").on('click',function(){
			window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/pay/ordersPay.html?orderIds=[' + ordersId + ']&soure=orders');
		});//立即支付
		$("#logistics").on('click',function(){
			window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/orders/orderEmsTrack/orderEmsTrack.html?orderId=' + ordersId);			
		});//查看物流		
	...
	function exit(){
		if(goodsId.length == 1){
			if(goodsId[0].status == 1){
				...
				window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/orders/returngoodsOrders/returngoodsform.html?goodsId=' + goodsId[0].id + '&ordersId=[' + ordersId + ']');
			}else{
				$.errtoast('该商品已在退换货中');
			}
			return;
		        ...
		        },
		        {
		        	...
		        	onClick: function() {
						...
						if(_Id == undefined){
							...
							window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/orders/returngoodsOrders/returngoodsform.html?goodsId=' + _Id + '&ordersId=[' + ordersId + ']');
						}
		          	}
		        }
			]					
	...
	function evaluate(){
		if(goodsId.length == 1){
			window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/comment/goodsAddComment.html?goodsId=' + goodsId[0].id + '&ordersId=' + ordersId);
			return;
		}	
		var _text = '';
		for(var i = 0; i < goodsId.length; i++){
		        ...
		        },
		        {
		        	...
		        	onClick: function(){
						...
						if(typeof(_Id) === 'undefined'){
							...
							window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/comment/goodsAddComment.html?goodsId=' + _Id + '&ordersId=' + ordersId);
						}
		          	}
		        }
			]					
