define(function(require, exports, module) {
	...
	window.jsObj.setLoadUrlTitle('预定预约订单详情');
	...
	ajax.get('preorders/preordersdetail/' + customerId + '/' + ordersId,'json',function(data){
		...
		for(var k = 0; k < goodsList.length; k ++){
			...
			lidata += '<a href="javascript:;" data-url="' + SAYIMO.SRVPATH + 'view/reserved/reservedDetail.html?goodsId=' + goodsList[k].id + '&normsValueId=' + goodsList[k].normsValueId + ahref + '" class="cont">' + 
			'	<div class="l">' + 
	...
	function statusBarBtn(){
		switch (status){
			case 1:
				...
				$(".bar").append('<a href="javascript:;" class="l" id="tel">联系商家</a>');	
				break;
			case 2:
	...
	function control(){
		$('.ment-list li a').on('click',function(){
		...
		$("#evaluate").on('click',function(){
			window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/comment/reservedAddComment.html?goodsId=' + goodsId + '&ordersId=' + ordersId);
		});//我要评价		
	}
