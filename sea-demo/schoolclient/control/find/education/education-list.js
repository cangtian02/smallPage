define(function(require, exports, module) {
	...
	window.jsObj.setLoadUrlTitle('职业课堂');
		...
		this.getEduList = function(j){
			...
			ajax.get('base/geteducationbytype/' + pageSize + '/' + pageNow + '/' + j,'json',function(data){
				if(data.status == 0){
					...
					$(".tab.active").html(base.noList('暂无' + t + '教学'));
				...
				if(data.educationList.length == 0){
					...
					if(pageNow == 1){
						$(".tab.active").html('<ul class="eduList">' + htmldata + '</ul>');
					}else{
						$(".tab.active").find(".eduList").append(htmldata);
					}
		...
		this.listClickFun = function(j){// 列表点击把数据使用localStorage本地缓存
			$("#tab" + j).find(".eduList li").each(function(){
				$(this).on('click',function(){
					...
					window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/find/education/education-detail.html?id=' + id);
				});
			});
		}
		...
		$(".tab-link:last-child").on('click',function(){
			...
			if(!$(this).hasClass('on')){								
				...
				$("#tab3 .eduList li").each(function(){
					$(this).on('click',function(){
						window.jsObj.loadContent(SAYIMO.SRVPATH + 'view/find/education/education-detail.html?id=' + $(this).attr('data-id'));
					});
				});				
			}
		});
