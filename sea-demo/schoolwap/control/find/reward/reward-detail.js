define(function(require, exports, module) {
							...
							$.confirm('您已投过标，是否更新投标？',
								function () {
									ajax.get('base/myrewardsubmitlist/' + customerId + '?rewardId=' + rewardId,'json',function(data){
										...
										window.location.href = SAYIMO.SRVPATH + 'view/find/reward/insertReward.html?id=' + rewardId + '&isSubmit=' + data.data.myRewardSubmitList.reward[0].id;
									});										
								}
							);					
						}else{
							
							window.location.href = SAYIMO.SRVPATH + 'view/find/reward/insertReward.html?id=' + rewardId + '&isSubmit=0';
						}						
					});
	...
	$(".tab-link").eq(1).on('click',function(){
		if(flag_1) return;
		ajax.get('base/myrewardsubmitlist/' + customerId + '?status=' + status + '&rewardId=' + rewardId,'json',function(data){
			if(data.status == 0){
				$.errtoast('服务器繁忙，请稍后重试');
				return;
			}
