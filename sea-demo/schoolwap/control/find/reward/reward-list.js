define(function(require, exports, module) {
	...
	var fun_rewardList = function(){
		...
		this.getRewardList = function(){
			ajax.get('base/selectbaserewardlist?id=' + '&classId=' + classId + '&pageSize=' + pageSize + '&pageNow=' + pageNow + '&rewardName=' + rewardName ,'json',function(data){
				if(data.status == 0){
					$.errtoast('服务器繁忙，请稍后重试');
					return;
				}
...
//									'		<div class="p_r">学 员<br>专 属</div>' +
		...
		$("#myreward").on('click',function(){
			window.location.href = SAYIMO.SRVPATH + 'view/find/reward/myreward-list.html';
		});
	}
	fun_rewardList();	
});