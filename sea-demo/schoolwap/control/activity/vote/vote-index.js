define(function(require, exports, module) {
	...
	var customerId = cookie.getCookie('customerId'),
		voteSubmissionId = base.getQueryString('id'),
	...
	ajax.get('base/commonvotedetail/' + voteSubmissionId,'json',function(data){
		if(data.status == 0){
			...
			for(var i = 0; i < data.length; i++){
				...
				if(identifier == 'SYDS'){
					for(var j = 0; j < data[i].photoUrl.length; j++){
						img += '<img class="lazy" data-lazyload="' + data[i].photoUrl[j].photoPath + '" src="' + SAYIMO.SRVPATH + 'images/default/sydefault.png">';
					}
					img = '<div class="img">' + img + '</div>';
				}
				htmldata += '<li>' +
							'	<div class="box">' +
							'	<a class="l" href="' + SAYIMO.SRVPATH + 'view/activity/vote/vote-detail.html?submissionId=' + data[i].submissionId + '&voteSubmissionId=' + voteSubmissionId + '&identifier=' + identifier + '">' +
							'		<img class="lazy" data-lazyload="' + data[i].headPhoto + '" src="' + SAYIMO.SRVPATH + 'images/default/sydefault.png" />' +
							'		<p><span>' + data[i].code + '</span>号</p>' +
							'	</a>' +
							'	<div class="r">' +
							'       <a href="' + SAYIMO.SRVPATH + 'view/activity/vote/vote-detail.html?submissionId=' + data[i].submissionId + '&voteSubmissionId=' + voteSubmissionId + '&identifier=' + identifier + '">' +
