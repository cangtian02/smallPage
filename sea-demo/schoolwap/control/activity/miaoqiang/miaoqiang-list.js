define(function(require, exports, module) {
	...
	getAcinfo(_tab);
	getAjax(identifier[_tab],_tab);
	
	$(".buttons-tab .button").each(function(){
		$(this).on('click',function(){
			var i = $(this).index();
			window.history.replaceState({title: "",url: ""}, "秒抢专区", SAYIMO.SRVPATH + 'view/activity/miaoqiang/miaoqiang-list.html?tab=' + i);			
			getAcinfo(i);
		...
		ajax.get('activity/ selectbaseseckillgoodsidentifier/' + identifier[j] + '/' + id +'/' + j,'json',function(data){		
			...
			if(j == 0){
				for(var i = 0; i < classList.length; i++){
					htmldata += '<li>' +
							'	<a href="' + SAYIMO.SRVPATH + 'view/reserved/reservedDetail.html?goodsId=' + classList[i].goodsId + '&normsValueId=' + classList[i].normsValueId + '&isActivity=1&identifier=' + identifier[j] + '&seckillTimesId=' + id + '">' +
				...
				for(var i = 0; i < classList.length; i++){
					htmldata += '<li>' +
							'<a class="cont"  href="' + SAYIMO.SRVPATH + 'view/class/goodsDetail.html?goodsId=' + classList[i].goodsId + '&normsValueId=' + classList[i].normsValueId + '&isActivity=1&identifier=' + identifier[j] + '&seckillTimesId=' + id + '">' +
