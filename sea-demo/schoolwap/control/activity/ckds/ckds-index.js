	...
	ajax.get('base/submissiondetail/' + submissionId,'json',function(data){
			...
			if(data.length > 0){
				...
				if(identifier == 'WJZJYJH'){
					for(var i = 0; i < data.length; i++){
						...
						var headPhoto = data[i].headPhoto == undefined ? '' : data[i].headPhoto;
						htmldata += '<li>'+
									'	<div class="l"><img class="lazy" data-lazyload="' + headPhoto + '" src="' + SAYIMO.SRVPATH + 'images/default/sydefault.png" /></div>' +
									'	<div class="r">' +
					...
					for(var i = 0; i < data.length; i++){
						...
						for(var j = 0; j < data[i].photoUrl.length; j++){
							img += '<img class="lazy" data-soure="' + data[i].photoUrl[j].photoPath + '" data-lazyload="' + data[i].photoUrl[j].minPhotoPath + '" src="' + SAYIMO.SRVPATH + 'images/default/sydefault.png">';
						}
						var headPhoto = data[i].headPhoto == undefined ? '' : data[i].headPhoto;
						htmldata += '<li>'+
									'	<div class="top">' +
		...
		$(".ljtgBtn").on('click',function(){
			if(submissionStatus == 0){
				...
				if(isFlag == true){
					...
					window.location.href = SAYIMO.SRVPATH + 'view/activity/ckds/ckds-ljtg.html?id=' + data.id + '&identifier=' + identifier;
				}			
		...
		$(".ljtpBtn").on('click',function(){
			if(submissionStatus == 0){
				...
				window.location.href = SAYIMO.SRVPATH + 'view/activity/vote/vote-index.html?id=' + submissionId + '&identifier=' + identifier;
			}else if(submissionStatus == 4){
				$.errtoast('活动已结束');
			}			
		});
