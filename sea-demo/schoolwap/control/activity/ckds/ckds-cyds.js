define(function(require, exports, module){
	...
	ajax.get('base/selectbasestartupgamebyid/' + id + '/' + customerId,'json',function(data){
		...
		base.setTitle(data.baseStartupGame.activityName);
		...
		$(".teamList li").each(function(i){
			...
			_this.find('.libm').on('click',function(){
				...
				window.location.href = SAYIMO.SRVPATH + 'view/activity/ckds/cyds-ljbm.html?gameId='+ _this.attr('data-gameId') + '&teamId=' + _this.attr('data-id');
			});
			_this.find('.cktdqd').on('click',function(){
				window.location.href = SAYIMO.SRVPATH + 'view/activity/ckds/cyds-list.html?teamId='+ _this.attr('data-id');
			});	
		});
	});
