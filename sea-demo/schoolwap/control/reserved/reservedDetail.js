define(function(require, exports) {
	...
	var fun_goodsDetail = function(){
		...
		ajax.get('pregoods/getgoodsinfo/' + goodsId + '/' + b_normsValueId + '?isActivity=' + isActivity + '&identifier=' + identifier + '&seckillTimesId=' + seckillTimesId + '&customerId=' + customerId,'json',function(data){
			if(data.status == 0){
				$.errtoast('系统繁忙，请稍后重试');
				return;
			}								
		...
		this.setBuybar = function(){
			...
			buybar({
				...
				serviceCall: function(){
					...
					window.location.href = SAYIMO.KFURL + '&goods=true&goodspic=' + photoUrls[0].url + '&goodsname=' + goodsName + '&goodsprice=' + totalprices + '元&goodsurl=' + encodeURIComponent(SAYIMO.SRVPATH + 'class/goodsDetail.html?goodsId=' + goodsId + '&normsValueId=' + b_normsValueId + ahref);
				},
		...
		this.popupbtncallback = function(){
			...
			if(popupId == 1){
				...
				window.location.href = SAYIMO.SRVPATH + 'view/pay/reservedPay.html?goodsId=' + goodsId + '&normsValueId=' + normsValueId + '&buyNum=' + Number + phref;			
			}
		}
		...
		this.addOrders = function(){
			...
			ajax.get('user/getreceiveaddress/' + customerId,'json',function(data){
				...
				if(data.data.addresses.length == 0){						
					$.confirm('<div style="font-size:.7rem;">您还没有收获地址,是否去添加？</div>',
						function () {
		...
		this.CommentList = function(){
			ajax.get('goods/getgoodscomments/' + goodsId + '?recordNum=5' + '&goodsType=PRE','json',function(data){				
				...
				if(data.commentCount > 0){				
					$(".comment-box").append('<div class="commentmore tc"><a href="' + SAYIMO.SRVPATH + 'view/comment/commentList.html?goodsId=' + goodsId + '&goodsType=pre">查看更多...</a></div>');					
