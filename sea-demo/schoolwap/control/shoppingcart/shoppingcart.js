define(function(require, exports, module) {
	...
	ajax.get('goods/getshoppingcartlist/' + customerId,'json',getList);
	...
	function getList(data){
		...
		var cartList = data.data.shoppingCartList;
		if(cartList.length == 0){
			$(".content").html('<div class="noCart"><img src="' + SAYIMO.SRVPATH + 'images/default/shopcare.png" /><p>您的购物车暂时无商品</p><a href="' + SAYIMO.SRVPATH + 'view/home/home.html" class="btn">继续逛一逛</a></div>');								
			return;
		}
		var htmldata = '',
			boxdata = '',
		...
		for(var i = 0; i < cartList.length; i++){				
			...
			for(var j = 0; j < goodsList.length; j++){					
				...
				listdata += '<li data-id = "' + goodsList[j].shoppingCardId + '" data-salePrice = "' + goodsList[j].salePrice + '" data-number = "' + goodsList[j].buyNum + '" data-total = "' + total + '" data-stock = "' + goodsList[j].goodsStockCount + '">' + 
					...
					'		<a class="fl" href="' + SAYIMO.SRVPATH + 'view/class/goodsDetail.html?goodsId=' + goodsList[j].id + '&normsValueId=' + goodsList[j].normsValueId + '"><img class="lazy" data-lazyload="' + goodsList[j].photoUrl + '" src="' + SAYIMO.SRVPATH + 'images/default/sydefault.png" /></a>' + 
					'	</div>' + 
					'	<div class="r">' + 
					'		<a href="' + SAYIMO.SRVPATH + 'view/class/goodsDetail.html?goodsId=' + goodsList[j].id + '&normsValueId=' + goodsList[j].normsValueId + '">' + 
			...
			boxdata = '<div class="box-title">' + 
				...
				'<div class="providerName ellipsis">' + cartList[i].providerName + '</div>' + 
			'</div>';				
			htmldata += '<div class="cart-box">' + boxdata + listdata + '</div>';
			listdata = '';
			boxdata = '';
		}						
		$('.content').html('<div class="cartList">' + htmldata + '</div>');
		$(".bar-tab~.content").css('bottom', '4.7rem');
		$(".page").append('<!--结算bar--><div class="account-bar"><div class="check fl" id="a-check"><i></i></div><span class="check-text">全选</span><div class="go fr" id="buy-go">去结算(<span id="a-number">0</span>)</div><div class="total fr">合计：<span class="red"><i class="i arial">￥</i><i id="a-total">0.00</i></span></div></div>');							
		laz.init();//刷新图片懒加载
		control();//控制层
	}
	...
	var control = function(){
		...
		require('touch');//引入zepto touch 实现swipeLeft,swipeRight
		...
		$(".cart-box").each(function(){
			...
			$(this).find('li').each(function(){
				$(this).swipeLeft(function(){
					$(this).find('.list-item').css('left','-2.5rem');
				});
				$(this).swipeRight(function(){
					$(this).find('.list-item').css('left','0');
				});				
			});			
		});
				...
				function () {
					ajax.post('goods/deleteshoppingcarts',{"shoppingCartIds": cartId},'json',function(data){
						...
						if($(".cartList li").length == 0){
							...
							$(".content").html('<div class="noCart"><img src="' + SAYIMO.SRVPATH + 'images/default/shopcare.png" /><p>您的购物车暂无商品</p><a href="' + SAYIMO.SRVPATH + 'view/home/home.html" class="btn">继续逛一逛</a></div>');							
						}else{
							self.fun_flag();//判断是否全选
							self.fun_account();//清算数据
						}						
		...
		this.fun_gobuy = function(){//结算事件
			...
			ajax.get('user/getreceiveaddress/' + customerId,'json',function(data){				
				...
				if(data.data.addresses.length == 0){
					...
					var areaCode = data.data.addresses[0].areaCode,				
						shoppingCartIds = [],
						buyNums = [];
					...
					ajax.post('order/insertordersbyshoopingcart',{
						...
						window.location.href = SAYIMO.SRVPATH + 'view/pay/ordersPay.html?ordersId=[' + data.data.orderId + ']&shopcart=1';//跳转订单支付页面	
					});						
				}
			});		
		}//fun_gobuy end
