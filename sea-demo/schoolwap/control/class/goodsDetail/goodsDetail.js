define(function(require, exports, module) {
	...
	var fun_goodsDetail = function(){
		...
		var	isActivity = base.getQueryString('isActivity') == null ? 0 : base.getQueryString('isActivity'),//是否是活动
			periodId = base.getQueryString('periodId') == null ? 0 : base.getQueryString('periodId'),
		...
		ajax.get('user/getreceiveaddress/' + customerId,'json',function(data){//获取用户默认收货地址code
			...
			ajax.get('goods/getgoodsinfo/' + goodsId + '/' + normsValueId + '?isActivity=' + isActivity + '&identifier=' + identifier + '&seckillTimesId=' + seckillTimesId + '&customerId=' + customerId + '&areaCode=' + '&isStudent=' + isStudent,'json',function(data){
				console.log(data);
				if(data.status == 0){
					$.errtoast('系统繁忙，请稍后重试');
					return;
		...
		this.renderDom = function(data){				
			...
			if(isStudent == 1){//学生店铺
				...
				$("#gostudentshop").attr('href', SAYIMO.SRVPATH + 'view/find/studentShop/studentShop-shopindex.html?providerId=' + data.providerId);
			}
		...
		this.setBuybar = function(){
			...
			buybar({
				...
				serviceCall: function(){
					...
					window.location.href = SAYIMO.KFURL + '&goods=true&goodspic=' + photoUrls[0].url + '&goodsname=' + goodsName + '&goodsprice=' + totalprices + '元&goodsurl=' + encodeURIComponent(SAYIMO.SRVPATH + 'view/class/goodsDetail.html?goodsId=' + goodsId + '&normsValueId=' + normsValueId + ahref);
				},
		...
		this.popup_box_norm_info = function(){
			...
			ajax.get('goods/getnormsinfo/' + goodsId + '/' + seVid + '?isActivity=' + isActivity + '&identifier=' + identifier + '&seckillTimesId=' + seckillTimesId + '&customerId=' + customerId,'json',function(data){
				...
				if(isActivity != 1){
					...
					totalprices = (preferentialPrice * buyNumber).toFixed(2);//规格价，根购买数量设置价格	
		...
		this.numbox = function(){
			...
			$(".popup-box-buy .numbox").numbox({
				...
				callNum : function(){
					...
					totalprices = (preferentialPrice * inputBoxVal).toFixed(2);	 //根购买数量设置价格				
					$(".popup-box-price-money").text(totalprices);//设置规格价	
		...
		this.CommentList = function(){
			ajax.get('goods/getgoodscomments/' + goodsId + '?recordNum='  + 5 + '&goodsType=','json',function(data){				
				if(data.status == 0){
				...
				if(data.commentCount > 0){									
					$(".comment-box").append('<div class="commentmore tc"><a href="' + SAYIMO.SRVPATH + 'view/comment/commentList.html?goodsId=' + goodsId +'">查看更多...</a></div>');				
