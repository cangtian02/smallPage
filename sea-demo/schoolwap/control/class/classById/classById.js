define(function(require, exports, module) {
	...
	var customerId = cookie.getCookie('customerId'),//会员id
		...
		providerId = (base.getQueryString("providerId")) == null ? '' : (base.getQueryString("providerId")),
		timestamp = (base.getQueryString("timestamp")) == null ? '' : (base.getQueryString("timestamp")),
	...
	function getAjax(i){//ajax获取数据 i = 1为页面初次载入和无限加载方式， i = 2 为下拉加载		
		...
		ajax.get('goods/getgoodslistbyclassid/' + classId + '/' + pageSize + '/' + pageNow + '?goodsName=' + goodsName + '&sortWord=' + sortWord + '&sortMode=' + sortMode + '&brandId=' + '&0' + '&providerId=' + providerId + '&timestamp=' + timestamp,'json',f);		
	}//getAjax end
	...
	function getListpull(data){
		...
		if(classList.length > 0){
			...
			if(pageNow == 1){
				...
				$("#classGoodList").prepend(classGoodsList(moduleData,pageNow));
			}
			laz.init();//刷新图片懒加载			
			classfun();
		}
		...
		$(".classGoodList li").each(function(){
			$(this).on('click',function(e){
				...
				var goodsdeail = SAYIMO.SRVPATH + 'view/class/goodsDetail.html?goodsId=' + goodsId + '&normsValueId=' + normsValueId,
					goodslist = SAYIMO.SRVPATH + 'view/class/classById.html?id=' + classId + '&pageSize=' + pageSize + '&pageNow=' + _pageNow + '&sortWord=' + sortWord + '&sortMode=' + sortMode + '&brandId=' + brandId + '&goodsName=' + goodsName + '&goodsId=' + goodsId + '&stop=' + stop + '&name=' + name;
